...
const FormEditPegawaiPage = () => {
  ...
  const getDataPegawaiById = async () => {
    try {
      const response = await ServicePegawai.getPegawaiById(IDPEGAWAI);
      setNama(response.data.NAMA)
      setEmail(response.data.EMAIL)
      setTelp(response.data.NO_TELP)
      setJabatan(response.data.IDJABATAN)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await ServicePegawai.editPegawai(IDPEGAWAI, data);
  ...
  return (
    <Layout>
      <div className="flex flex-col gap-3 px-5 my-2 border-lg shadow w-full py-3 bg-white">
        <p className="text-lg font-bold uppercase">Update Data Pegawai</p>
        <hr />
...
export default FormEditPegawaiPage;
